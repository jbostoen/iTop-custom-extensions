	...
	public static function EnumItems($iMenuId, $param)
	{
		...
		if ($iMenuId == self::MENU_OBJDETAILS_ACTIONS)
		{
			...
			foreach( $aReportFiles as $sReportFile ) 
			{
				...
				$sURL = utils::GetAbsoluteUrlAppRoot() . '/extensions/jb-reportgen/showreport.php?type=detail&class=' . get_class($oObject) . '&key=' . $oObject->Get('id') . '&template=details/' . basename(basename($sReportFile));
		...
		elseif($iMenuId == self::MENU_OBJLIST_ACTIONS)
		{
			...
			// Where are reports located?		
			$sClassReportDir = APPROOT . '/extensions/jb-reportgen/templates/'. get_class($aObjectSet[$aKeys[0]]); 
			...
			foreach( $aReportFiles as $sReportFile ) 
			{
				...
				// type=Object is to allow 'showReport.php' to also include lists in the future.
				$sURL = utils::GetAbsoluteUrlAppRoot() . '/extensions/jb-reportgen/showreport.php?type=list&class=' . get_class($oObject) . '&keys=' . implode(',' , $aKeys) . '&template=list/' . basename(basename($sReportFile));
