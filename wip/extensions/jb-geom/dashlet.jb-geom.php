...
EOF
			...
			$oPage->add_ready_script('
				...
				geometryHandler_'.$sId.'.aFeatureSet = '.json_encode($aFeatures).';
				
				$.each(geometryHandler_'.$sId.'.aFeatureSet, function(i) {
					// Must contain geometry
					if(geometryHandler_'.$sId.'.aFeatureSet[i].geometry != "") {
						var oFeature = geometryHandler_'.$sId.'.oFormat.'.$aGeomSettings['dataformat'].'.readFeature(geometryHandler_'.$sId.'.aFeatureSet[i].geometry, { dataProjection: "'.$aGeomSettings['datacrs'].'", featureProjection: "'.$aGeomSettings['mapcrs'].'" });
				...
				geometryHandler_'.$sId.'.oMap = new ol.Map({
					...
					layers: [
						...
						geometryHandler_'.$sId.'.aLayers.osm,
