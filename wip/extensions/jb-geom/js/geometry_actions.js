 ...
 */
	...
	geometryHandler["common"].Show_OpenStreetMap = function (){
		...
		var aCoords = ol.proj.transform(geometryHandler["common"].oCenter, geometryHandler["common"].oView.getProjection(), 'EPSG:4326');
	...
	geometryHandler["common"].Copy_As_GeoJSON = function (){
		...
		// GeoJSON is by default EPSG:4326
		var oView = geometryHandler["common"].oMap.getView();
		var oFeature = geometryHandler["common"].oFeature.clone();
		var aCoords = ol.proj.transform(oFeature.getGeometry().getCoordinates(), geometryHandler["common"].oMap.getView().getProjection(), 'EPSG:4326');
		$('#geometryHandler_GeoJSON').html(geometryHandler["common"].oFormat.GeoJSON.writeFeature(oFeature)).show().select();
