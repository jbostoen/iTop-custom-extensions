 ...
 *
 * Definition of ApplicationUIExtensionCobra
 */

namespace jb_itop_extensions\cobra;

/**
* Class ApplicationUIExtensionCobra. Adds tab with map, makes textbox 'geom' invisible
*/
class ApplicationUIExtensionCobra implements \iApplicationUIExtension
{
	/**
	...
	*/
	...
	/**
	* Invoked when an object is being displayed (view or edit)
	*
	...
	*/
	public function OnDisplayProperties($oObject, \WebPage $oPage, $bEditMode = false) : void {
		...
		if($bEditMode == false && in_array('id_agenda_item', $aAttributeList) == true) {
			
			\jb_itop_extensions\cobra\CobraHelper::AddCobraDetails($oPage, $oObject->Get('id_agenda_item'));			
